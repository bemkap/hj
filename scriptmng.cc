...
pair<string,object*>cscriptmng::loadobj(const char*f){
  luaL_dofile(L,f);
  object*o=new object;
  string n;
  if(lua_istable(L,-1)){
    ...
    while(lua_next(L,-2)!=0){
      ...
      if(s=="spr") o->sprite=env.sprites.get(lua_tostring(L,-1));
      if(s=="name") n=lua_tostring(L,-1);
